...
class CustomLevelheadComponent: UIComponent() {
    ...
    private fun UIComponent.createComponents() {
        ...
        var currentProposal by Delegates.notNull<JsonObject>()
        var proposalHeader by Delegates.notNull<UIText>()
        var proposalFooter by Delegates.notNull<UIText>()
        Levelhead.scope.launch {
            currentProposal = getProposalInfo()
        }.invokeOnCompletion {
            Window.enqueueRenderOperation {
                ...
                val headerColorComponent = (currentProposal["current"]?.asJsonObject?.get("header_obj")?.asJsonObject?.let { headerObj ->
                ...
                val footerColorComponent = (currentProposal["current"]?.asJsonObject?.get("footer_obj")?.asJsonObject?.let { footerObj ->
